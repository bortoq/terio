...
- [~] `terio_execute` safety: basic risk check + confirmation для Destructive/NetworkWrite
- [x] Builtin Rhai/TOML скрипты существуют
- [~] Input routing через ScriptEngine: help/mode/focus/scroll/repeat/ask проходят через scripts (с fallback на hardcoded)
- [x] `terio script install <file>`, `terio script list`
- [~] Override: по id + по trigger priority (User > Core > Learned > Builtin)
