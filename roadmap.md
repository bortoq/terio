...
- [x] CI: scripts/package.sh (build + package)
...
- [x] Per-entry risk cost в cost report
- [x] Route optimizer function (cost-aware, integrated in debug mode)
- [ ] Route optimizer fully integrated into `ask` production flow

## Фаза 6. Desktop + сообщество

- [x] Локальная LLM — Ollama provider (baseline, experimental)
- [x] Desktop-пакет (scripts/package.sh, baseline)
- [x] Автообновление (scripts/update.sh, baseline, unsigned)
- [x] Экспорт/импорт скриптов (`terio script export/import`, baseline)
- [x] Реестр скриптов сообщества (`terio registry search/install/publish/inspect`, baseline)
- [ ] Production desktop package (signed, dependencies, .desktop)
- [ ] Signed auto-update (checksum + GPG)
- [ ] Trusted registry protocol (provenance, signatures)
...
- [x] Написать группировщик: связывает Ask → ответ в пару по `interaction_id`
...
- [x] JSONL-формат хранения НЕ меняется (обратная совместимость)
- [x] Обновить архитектурную документацию

## Фаза 8. Маршрутизация без внутренних команд

**Цель:** убрать все hardcoded внутренние команды terio (`help`, `mode`, `focus`, `scroll`, `exit`, `undo`, `redo`). Весь пользовательский ввод обрабатывается единым pipeline:

```
ввод → ScriptEngine (поиск скрипта по триггеру)
        ├── скрипт найден → выполнить → показать окно
        └── скрипт не найден → LLM (ask)
                ├── успех → показать окно
                └── Unknown/недоступен → shell fallback (sh -c) → показать окно
```

### Изменения в коде

- [ ] `parse_input` — убрать все внутренние команды, оставить только проверку триггеров скриптов + shell
...
- [ ] tests: обновить `test_parse_input_*` на новую логику
