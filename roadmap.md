...
- [x] Normalize запроса → HashMap<NormalizedQuery, ScriptId>
- [x] Автоматическое создание синонимов из успешных LLM-запросов (CacheHit/FromAgent)
- [x] Редко используемые синонимы → prune() из индекса
- [x] `terio alias list` / `terio alias remove` CLI

## Фаза 4. Проактивный режим (предугадывание)

- [x] terio смотрит историю и предлагает следующую команду (n-gram модель из лога)
- [x] Автодополнение: `# terio: <request>?` после каждого ask
- [x] Молчаливое исполнение при точности > 0.95 в quiet mode
- [x] Счётчик авто-выполненных команд: «terio: +N auto-executed»

## Фаза 5. Оптимизация стоимости

- [x] Формула: C_total = C_llm_tokens + C_user_attention + C_risk (compute_total_cost с учётом config.cost)
- [x] Выбор маршрута: скрипт (дёшево) vs LLM (гибко) (RouteDecision + select_route)
- [x] `terio cost` — отчёт по затратам (читает лог, агрегирует, показывает C_total + экономию)
- [x] Байесовский классификатор для точности предсказаний (BayesianPredictor с prior + likelihood)
