...
- [x] Реальный route optimizer (cost-aware)

## Фаза 6. Desktop + сообщество

- [x] Локальная LLM (Ollama HTTP provider)
- [x] Desktop-пакет (scripts/package.sh) + автообновление (scripts/update.sh)
- [x] Экспорт/импорт скриптов (`terio script export/import`)
- [x] Реестр скриптов сообщества (`terio registry search/install/publish`)
