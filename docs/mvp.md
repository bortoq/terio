...
- Hybrid model: initial snapshot через `recent(N)` + live in-process updates через `LogStore` broadcast.
- Plain text / Readable page.
- Table (ls, csv-подобный вывод).
- Timeline, Card, Chat renderers поверх той же event-ленты.
- Авто-определение renderer на основе `display_profile`.
...
- Для `terio ask` и cache-script replay.
- `terio run -- ...` вне snapshot/undo guarantees.
- Sandbox (`bubblewrap`) или warn (конфиг).
- Off by default.
