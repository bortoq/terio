...
**UI с первого коммита:** terio — это оконное приложение (одно окно, Dioxus webview). Пользователь не набирает команды в raw shell, а взаимодействует с terio через его UI. Shell-команды (`terio run --`) доступны для обратной совместимости, но основной путь — `terio ask` с окном.
...
### 2. `terio run -- <command>` — shell-команда (дополнительно)
- Прямая команда, тоже логируется и показывается в UI.
- Для обратной совместимости и простых задач.
...
- В будущем — полноценная оконная система (Phase 4+).
...
- ❌ **Desktop (standalone-пакет, system tray, автообновление).** Dioxus webview — не Desktop, а встроенное окно.
