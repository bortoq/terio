...
Считаются все доступные метрики: вызовы модели, попадания в кеш, длительность, ошибки. Это счётчики — небольшой объём данных. Агрегация — по запросу (`terio stats`). Всё, что можно пересчитать из лога, пересчитывается по запросу. Главное — ничего не терять из поступающей статистики.
